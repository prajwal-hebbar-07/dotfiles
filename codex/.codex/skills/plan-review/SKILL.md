...
inline `@me` comments, apply them, and rewrite the file cleanly. The nvim
split the user is commenting in autoreloads the rewrite, so the loop never
leaves the terminal.
...
7. **Write the file back.** nvim has `autoread` on and runs `checktime` on
   CursorHold, so the split reloads on its own — the user does not need to
   do anything.

8. **Report briefly** in this pane: how many comments were resolved, one
   line each (same content as the changelog round). Then STOP and wait for
   the next round, or for the user to explicitly ask to implement.
