---
name: plan-done
description: Retire a plans/plan-<slug>.md file created by /plan once its plan has actually been implemented — verify the work landed (working tree or git history), then delete the plan file and clean up an empty plans/ dir. Refuses to delete unimplemented plans without explicit confirmation. Use when the user says "/plan-done", "the plan is implemented, delete it", "clean up the plan file", or "remove the plan, it's done".
---
...
3. **Delete the file.** `git rm` if tracked (leave the deletion uncommitted
   for `/ship`), plain `rm` if not. If the file has unsaved `@me` comments,
   mention they existed — they die with the file.
...
- **Never commit or push** — deletion stays in the working tree for `/ship`.
