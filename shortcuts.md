...
The Herdr prefix is configured to match tmux:
...
| `prefix` then `c` | Create a new tab |
| `prefix` then `,` | Rename the current tab |
| `prefix` then `Shift-x` | Close the current tab |
| `prefix` then `Shift-p` | Rename the current pane |
| `prefix` then `x` | Close the current pane |
| `prefix` then `m` | Zoom or restore the current pane |
| `prefix` then `g` | Open `lazygit` in a temporary pane |
| `prefix` then `y` | Open `yazi` in a temporary pane |
| `prefix` then `t` | Show a three-level `tree` view |
| `prefix` then `h` | Move to the pane on the left |
| `prefix` then `j` | Move to the pane below |
| `prefix` then `k` | Move to the pane above |
| `prefix` then `l` | Move to the pane on the right |
...
| `Ctrl` + `k` | Move to the pane above without pressing prefix |
| `Ctrl` + `l` | Move to the pane on the right without pressing prefix |
| `prefix` then `H` | Resize pane left |
| `prefix` then `J` | Resize pane down |
| `prefix` then `K` | Resize pane up |
| `prefix` then `L` | Resize pane right |
| `prefix` then `\` | Split pane to the right |
| `prefix` then `-` | Split pane downward |

Herdr also keeps its built-in keyboard copy mode on `prefix` then `[`. Inside
copy mode, use Vim movement, `v` or `Space` to start selection, and `y` or
`Enter` to copy.
